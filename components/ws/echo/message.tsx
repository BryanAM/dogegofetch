...
export default function Message(message: MessageType) {
  return (
    <article className="my-2 flex flex-col">
      <p
        className={`text-muted-foreground ${message.person === "me" && "text-right"}`}
      >
        {message.person}
      </p>
      <p
      ...
      <span
        className={`text-xs text-muted-foreground/50 opacity-0 transition-all duration-300 ease-in-out peer-hover:opacity-100 ${message.person === "me" && "text-right"}`}
      >
        {message.time}
      </span>
    </article>
